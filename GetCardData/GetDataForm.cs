...
namespace GetCardData
{
    public partial class GetDataForm : Form
    {
        ...
        public GetDataForm()
        {
            ...
            dataSourceTableAdapter.Fill(cardDBDataSet.DataSource);
            monsterTableAdapter.Fill(cardDBDataSet.Monster);
            spellTableAdapter.Fill(cardDBDataSet.Spell);
            trapTableAdapter.Fill(cardDBDataSet.Trap);
        ...
        private void LoadCardListItem(string path)
        {
            ...
            {
                var str = file.Remove(0, path.Length + 1).Replace(".jpg", "");
                var row = cardDBDataSet.DataSource.FindById(str);
        ...
        private void GetCardData(string cardId)
        {
            ...
            var row = cardDBDataSet.DataSource.FindById(cardId);
            if (row != null)
            {
            ...
            if (content != null)
            {
                ...
                if (content.SelectSingleNode("//table[@class='cardtable']") == null)
                {
        ...
        private void SaveCardData()
        {
            ...
            var rowedit = cardDBDataSet.DataSource.FindById(_cardDataTable["Id"]);
            if (rowedit != null)
            {
            ...
            var newcard = cardDBDataSet.DataSource.NewDataSourceRow();
            ...
            cardDBDataSet.DataSource.Rows.Add(newcard);
            cardDBDataSet.AcceptChanges();

            switch (_cardDataTable["CardType"])
            {
                case "MON":
                {
                    var newmonster = cardDBDataSet.Monster.NewMonsterRow();
                    newmonster.Id = _cardDataTable["Id"];
                    newmonster.IsEffect = _cardDataTable["IsEffect"] != "0";
                    newmonster.MonsterType = _cardDataTable["MonsterType"].ToUpper().Replace("-", "");
                    ...
                    cardDBDataSet.Monster.Rows.Add(newmonster);
                    cardDBDataSet.AcceptChanges();
                ...
                case "TRA":
                {
                    var newtrap = cardDBDataSet.Trap.NewTrapRow();
                    newtrap.Id = _cardDataTable["Id"];
                    newtrap.TrapType = _cardDataTable["TrapType"].ToUpper();

                    cardDBDataSet.Trap.Rows.Add(newtrap);
                    cardDBDataSet.AcceptChanges();
                ...
                case "SPE":
                {
                    var newspell = cardDBDataSet.Spell.NewSpellRow();
                    newspell.Id = _cardDataTable["Id"];
                    newspell.SpellType = _cardDataTable["SpellType"].ToUpper();

                    cardDBDataSet.Spell.Rows.Add(newspell);
                    cardDBDataSet.AcceptChanges();
                    break;
                }
            }
        ...
        private void FillCardInfo(string cardId)
        {
            ...
            var card = cardDBDataSet.DataSource.FindById(cardId);
            if (card == null) return;
            ...
            switch (card.CardType)
            {
                case "MON":
                    {
                        var monster = cardDBDataSet.Monster.FindById(card.Id);
                ...
                case "TRA":
                    {
                        foreach (var trap in cardDBDataSet.Trap)
                        {
                            if (trap.Id == card.Id)
                            {
                ...
                case "SPE":
                    {
                        foreach (var spell in cardDBDataSet.Spell)
                        {
                            if (spell.Id == card.Id)
                            {
        ...
        private void GetAllCard()
        {
            ...
            foreach (DataGridViewRow row in cardList.Rows)
            {
                var datarow = cardDBDataSet.DataSource.FindById(row.Cells[0].Value.ToString());
        ...
        {
            var cardRow = cardDBDataSet.DataSource.FindById(cardId);
            cardRow.BeginEdit();
            ...
            cardRow.EndEdit();
            cardDBDataSet.DataSource.AcceptChanges();

            switch (_cardDataTable["CardType"])
            {
                case "MON":
                {
                    var monsterCard = cardDBDataSet.Monster.FindById(cardRow.Id);
                    monsterCard.BeginEdit();
                    monsterCard.IsEffect =  _cardDataTable["IsEffect"] == "1";
                    monsterCard.MonsterType = _cardDataTable["MonsterType"].ToUpper().Replace("-", "");
                    ...
                    monsterCard.EndEdit();
                    cardDBDataSet.Monster.AcceptChanges();
                    break;
                }
                case "TRA":
                    {
                        foreach (var trap in cardDBDataSet.Trap)
                        {
                            if (trap.Id == _cardDataTable["Id"])
                            {
                                ...
                                trap.EndEdit();
                                cardDBDataSet.Trap.AcceptChanges();
                            }
                        }
                ...
                case "SPE":
                    {
                        foreach (var spell in cardDBDataSet.Spell)
                        {
                            if (spell.Id == _cardDataTable["Id"])
                            {
                                ...
                                spell.EndEdit();
                                cardDBDataSet.Spell.AcceptChanges();
                            }
                        }
                        break;
                    }
